    ...
    <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'"
        ...
        <nav class="flex-1 px-3 py-4 space-y-1 text-sm overflow-y-auto sidebar-scrollbar">
            @php
                ...
                $isHRD = $userRole === 'HRD';
                $isPayroll = $userRole === 'PAYROLL';
                $isCMS = $userRole === 'CMS';
                $isSuperAdmin = in_array($userRole, ['SUPERADMIN', 'HRD']);
                $isAdmin = in_array($userRole, ['SUPERADMIN', 'HRD', 'ADMIN', 'PROJECT_ADMIN']);
            @endphp
            ...
            <a href="{{ route('admin.permissions.index') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-slate-800 transition @if(request()->routeIs('admin.permissions.*')) bg-slate-800 @endif">
            ...
            <a href="{{ route('admin.reports.performance') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-slate-800 transition @if(request()->routeIs('admin.reports.performance')) bg-slate-800 @endif">
            ...
            <a href="{{ route('admin.leave-types.index') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-slate-800 transition @if(request()->routeIs('admin.leave-types.*')) bg-slate-800 @endif">
            ...
            <a href="{{ route('admin.leave-balance.index') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-slate-800 transition @if(request()->routeIs('admin.leave-balance.*')) bg-slate-800 @endif">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"/></svg>
                Saldo Cuti
            </a>

            <div class="pt-4 pb-1 text-xs font-semibold text-gray-400 uppercase tracking-wider px-3">Pemberitahuan</div>
            <a href="{{ route('admin.broadcast.index') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-slate-800 transition @if(request()->routeIs('admin.broadcast.*')) bg-slate-800 @endif">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"/></svg>
                Kirim Pemberitahuan
            </a>
            <a href="{{ route('admin.meetings.index') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-slate-800 transition @if(request()->routeIs('admin.meetings.*')) bg-slate-800 @endif">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z"/></svg>
                Meeting
            </a>
            @endif

            @if($isHRD || $isPayroll || $isSuperAdmin)
            ...
            <a href="{{ route('admin.psikotest.index') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-slate-800 transition @if(request()->routeIs('admin.psikotest.*')) bg-slate-800 @endif">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01"/></svg>
                Psikotest
            </a>
            @endif

            @if($isPayroll || $isHRD || $isSuperAdmin)
            <div class="pt-4 pb-1 text-xs font-semibold text-gray-400 uppercase tracking-wider px-3">Payroll</div>
            ...
            {{-- Karir & Pesan Masuk - Accessible by HRD, CMS, and SUPERADMIN --}}
            @if($isHRD || $isPayroll || $isCMS || $isSuperAdmin)
                @if(($isHRD || $isPayroll) && !$isSuperAdmin)
                <div class="pt-4 pb-1 text-xs font-semibold text-gray-400 uppercase tracking-wider px-3">Company Profile</div>
                @endif
            ...
            @if($isCMS || $isSuperAdmin)
            <div class="pt-4 pb-1 text-xs font-semibold text-gray-400 uppercase tracking-wider px-3">Pengaturan</div>
            <a href="{{ route('admin.cms-testimonials.index') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-slate-800 transition @if(request()->routeIs('admin.cms-testimonials.*')) bg-slate-800 @endif">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"/></svg>
                Testimoni
            </a>
            <a href="{{ route('admin.cms-faqs.index') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-slate-800 transition @if(request()->routeIs('admin.cms-faqs.*')) bg-slate-800 @endif">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8.228 9c.549-1.165 2.03-2 3.772-2 2.21 0 4 1.343 4 3 0 1.4-1.278 2.575-3.006 2.907-.542.104-.994.54-.994 1.093m0 3h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                FAQ
            </a>
            <a href="{{ route('admin.settings.index') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-slate-800 transition @if(request()->routeIs('admin.settings.*')) bg-slate-800 @endif">
