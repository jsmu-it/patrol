...
<div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs mb-6">
    ...
    <div>
        ...
        <select name="role" class="w-full border border-gray-300 rounded px-2 py-1.5" required>
            @php($peranTerpilih = old('role', $user->role ?? ''))
            @foreach(\App\Models\User::peranFormulir() as $kode => $label)
                <option value="{{ $kode }}" @selected($peranTerpilih === $kode)>{{ $label }}</option>
            @endforeach
            @if($peranTerpilih === \App\Models\User::ROLE_PROJECT_ADMIN)
                {{-- Peran lama; ditampilkan hanya agar akun yang masih memakainya tidak berubah diam-diam. --}}
                <option value="PROJECT_ADMIN" selected>PROJECT ADMIN (lama)</option>
            @endif
        </select>
    </div>
    <div>
