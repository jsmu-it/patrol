...
@section('page_title', 'Buat Meeting Baru')

@section('content')
<div class="max-w-2xl">
    <div class="bg-white rounded shadow">
        <div class="p-4 border-b">
            <h2 class="text-lg font-semibold">Buat Meeting Baru</h2>
            <p class="text-sm text-gray-500 mt-1">Isi form berikut untuk membuat meeting online baru.</p>
        </div>
        <form method="POST" action="{{ route('admin.meetings.store') }}" class="p-6 space-y-5">
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="3"
                    class="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:ring-blue-500 focus:border-blue-500"
                    placeholder="Deskripsi singkat tentang meeting...">{{ old('description') }}</textarea>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="number" name="max_participants" id="max_participants" value="{{ old('max_participants', 100) }}" min="2" max="500" required
                        class="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:ring-blue-500 focus:border-blue-500">
                </div>
            </div>
            ...
            <div class="border-t pt-4">
                ...
                <div class="space-y-3">
                    <label class="flex items-center gap-3 cursor-pointer">
                        <input type="checkbox" name="settings[mute_on_join]" value="1" {{ old('settings.mute_on_join') ? 'checked' : '' }}
                            class="rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                        <span class="text-sm text-gray-700">Mute mikrofon saat bergabung</span>
                    </label>
                    <label class="flex items-center gap-3 cursor-pointer">
                        <input type="checkbox" name="settings[disable_camera_on_join]" value="1" {{ old('settings.disable_camera_on_join') ? 'checked' : '' }}
                            class="rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                        <span class="text-sm text-gray-700">Matikan kamera saat bergabung</span>
                    </label>
                    <label class="flex items-center gap-3 cursor-pointer">
                        <input type="checkbox" name="settings[enable_lobby]" value="1" {{ old('settings.enable_lobby') ? 'checked' : '' }}
                            class="rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                        <span class="text-sm text-gray-700">Aktifkan ruang tunggu (Lobby)</span>
                    </label>
                    <label class="flex items-center gap-3 cursor-pointer">
                        <input type="checkbox" name="settings[enable_recording]" value="1" {{ old('settings.enable_recording') ? 'checked' : '' }}
                            class="rounded border-gray-300 text-blue-600 focus:ring-blue-500">
