...
<body>
    ...
    <table>
        <thead>
            <tr>
                <th colspan="{{ 1 + (count($allDates) * 2) }}" style="font-weight: bold; text-align: center; font-size: 14pt;">LAPORAN ABSENSI</th>
            </tr>
            <tr>
                <td colspan="{{ 1 + (count($allDates) * 2) }}" style="text-align: center;">Periode: {{ $filters['from']->format('d M Y') }} - {{ $filters['to']->format('d M Y') }}</td>
            </tr>
            <tr>
                <td colspan="{{ 1 + (count($allDates) * 2) }}" style="text-align: center;">Project: {{ $projectName }}</td>
            </tr>
            @if($userName)
            <tr>
                <td colspan="{{ 1 + (count($allDates) * 2) }}" style="text-align: center; font-weight: bold;">Karyawan: {{ $userName }}</td>
            </tr>
            @endif
            <tr>
